...
class CANBaseDriver(BaseDriver):
    ...
    def __init__(self, msgName, operation, msgID, extendedID=False, channel="can0", bitrate=250000):
...
class CANSender(CANBaseDriver):
    ...
    def __init__(self, msgName, msgID, extendedID=False, channel="can0", bitrate=250000):
        super().__init__(msgName, "send", msgID, extendedID, channel, bitrate)
        self.bus = can.interface.Bus(channel=self.channel, bustype='socketcan', bitrate=self.bitrate)

    def send(self, data):
        """Send a CAN message"""
        try:
            msg = can.Message(arbitration_id=self.msgID, data=data, is_extended_id=self.extendedID)
            self.bus.send(msg)
            print(f"Sent CAN message: ID={self.msgID}, Data={data}")
...
class CANReceiver(CANBaseDriver):
    ...
    def __init__(self, msgName, msgID, extendedID=False, channel="can0", bitrate=250000):
        super().__init__(msgName, "receive", msgID, extendedID, channel, bitrate)
        self.__filter = [{
            "can_id": self.msgID, 
            "can_mask": 0x7FF if not self.extendedID else 0x1FFFFFFF, 
            "extended": self.extendedID
        }]
        self.bus = can.interface.Bus(channel=self.channel, bustype='socketcan', bitrate=self.bitrate, can_filters=self.__filter)

    def receive(self):
        """Receive a CAN message"""
        msg = self.bus.recv(timeout=5.0)  # will wait 5 seconds for the messages before returning None
        if msg:
            print(f"Received CAN message: ID={msg.arbitration_id}, Data={msg.data}")
            return msg
        else:
